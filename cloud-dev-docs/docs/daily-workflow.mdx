...
| Scroll up | Mouse scroll or `Ctrl+B` then `[` (arrows, `q` to quit) |
...
:::tip Copying text while mouse mode is on

Mouse scrolling works out of the box — the container sets `set -g mouse on` in `/config/.tmux.conf` automatically.

But mouse mode also captures text selection. To copy text out of tmux:

| Method | How |
|--------|-----|
| **Shift + select** (easiest) | Hold **Shift** while clicking and dragging — bypasses tmux, uses your terminal's native copy. Works in Terminal.app, Termius, ttyd, and most SSH clients |
| **tmux copy mode** | `Ctrl+B` then `[` → move with arrows → `Space` to start selection → `Enter` to copy → `Ctrl+B` `]` to paste inside tmux |
| **ttyd (browser)** | Hold Shift + select, or right-click → Copy. Toggle mouse-capture in the ttyd titlebar to disable tmux mouse entirely for that session |

Without `set -g mouse on`, scrolling in tmux cycles through shell history instead of scrolling output, which is a [common tmux gotcha](https://www.reddit.com/r/ClaudeCode/comments/1rdwpdb/terminus_not_scrolling/) and **not a Claude Code or Termius bug**. If you're using tmux outside Claude World, add `set -g mouse on` to your `~/.tmux.conf` and restart tmux.
