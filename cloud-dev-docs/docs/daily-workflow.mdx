...
Mouse scrolling works out of the box as the container sets `set -g mouse on` in `/config/.tmux.conf` automatically.

Without this setting, scrolling in tmux cycles through shell history instead of scrolling output, which is a [common tmux gotcha](https://www.reddit.com/r/ClaudeCode/comments/1rdwpdb/terminus_not_scrolling/) and **not a Claude Code or Termius bug**. If you're using tmux outside Claude World, add `set -g mouse on` to your `~/.tmux.conf` and restart tmux.

To select text while mouse mode is on, hold **Option** (macOS) or **Shift** (Linux) while clicking and dragging. This bypasses tmux mouse capture and uses your terminal's native selection.
